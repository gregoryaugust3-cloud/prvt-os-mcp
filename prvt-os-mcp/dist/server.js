...
function createServer() {
    ...
    server.tool('search_prvt', 'Search PRVT OS for projects, leads, partners, SOPs, decisions, or other records.', { query: z.string(), type: z.string().optional(), status: z.string().optional(), limit: z.number().int().min(1).max(50).default(20) }, async ({ query, type, status, limit }) => {
        const like = `%${query}%`;
        const rows = db.prepare(`SELECT * FROM items WHERE (title LIKE ? OR content LIKE ?) ${type ? 'AND type = ?' : ''} ${status ? 'AND status = ?' : ''} ORDER BY updated_at DESC LIMIT ?`)
            .all(...[like, like, ...(type ? [type] : []), ...(status ? [status] : []), limit]);
        return { content: [{ type: 'text', text: json(rows) }] };
    });
    server.tool('get_prvt_item', 'Retrieve one PRVT OS record by numeric ID.', { id: z.number().int() }, async ({ id }) => {
        const row = db.prepare('SELECT * FROM items WHERE id = ?').get(id);
        return { content: [{ type: 'text', text: json(row || null) }] };
    });
    server.tool('upsert_prvt_item', 'Create or update a PRVT OS record. Use for approved knowledge, decisions, SOPs, pipeline records, and project updates.', { id: z.number().int().optional(), type: z.string(), title: z.string(), status: z.string().default('active'), owner: z.string().default('Greg'), priority: z.string().default('normal'), content: z.string(), metadata: z.record(z.string(), z.any()).default({}) }, async (input) => {
    ...
    server.tool('list_prvt', 'List current PRVT OS records by type/status.', { type: z.string().optional(), status: z.string().optional(), limit: z.number().int().min(1).max(100).default(50) }, async ({ type, status, limit }) => {
        const rows = db.prepare(`SELECT id,type,title,status,owner,priority,updated_at FROM items WHERE 1=1 ${type ? 'AND type=?' : ''} ${status ? 'AND status=?' : ''} ORDER BY updated_at DESC LIMIT ?`)
            .all(...[...(type ? [type] : []), ...(status ? [status] : []), limit]);
        return { content: [{ type: 'text', text: json(rows) }] };
    });
    return server;
}
...
app.get('/health', (_req, res) => res.json({ status: 'ok', service: 'prvt-os-mcp' }));
// Remote MCP endpoint for Claude and other MCP clients.
// Stateless mode keeps this MVP simple and stores durable business data in SQLite.
...
app.get('/mcp', async (req, res) => {
    ...
    try {
        ...
        if (!res.headersSent)
            res.status(405).json({ error: 'MCP GET not available in stateless mode' });
    }
});
app.delete('/mcp', (_req, res) => res.status(204).end());
const port = Number(process.env.PORT || 10000);
app.listen(port, '0.0.0.0', () => {
    console.log(`PRVT OS MCP listening on 0.0.0.0:${port}`);
